...
public class TestDeadlockPrevention {
  ...
  public void no_Deadlock_from_queueing1() throws InterruptedException {
    final LockManager lockMan = new LockManager();
    AsyncDeadlockTesterThread thread1 = new AsyncDeadlockTesterThread(new Runnable() {
      public void run() {
        lockMan.acquireLock("A", 1, LockManager.LockType.EXCLUSIVE);
    ...
    try {
      ...
      assertFalse("thread1 shouldnt wait", thread1.isAlive()); //T2 should not be blocked on B
      assertTrue("thread2 shoudl wait", thread2.isAlive());
      assertTrue("thread3 should be blocked", thread3.isAlive());
    } catch (DeadlockException d) {
      fail("No deadlock exists but Deadlock Exception was thrown.");
    }
  ...
  @Test //thread3 shouldnt wait on thread2 which is in the queue, so thread2 waiting on thread3 shouldnt create deadlock
  @Category(StudentTestP3.class)
  public void no_Deadlock_from_queueing2() throws InterruptedException {
    final LockManager lockMan = new LockManager();
    AsyncDeadlockTesterThread thread1 = new AsyncDeadlockTesterThread(new Runnable() {
      public void run() {
        lockMan.acquireLock("A", 1, LockManager.LockType.EXCLUSIVE);
    ...
    try {
      ...
      thread4.test();
      assertFalse("Transaction 4 Thread should have finished", thread4.isAlive()); //T2 should be waiting on T1 for A

      thread5.start();
      thread5.join(100); //waits for thread to finish (timeout of .1 sec)
      thread5.test();
      assertTrue("thread5 should be blocked", thread5.isAlive());
    } catch (DeadlockException d) {
      fail("No deadlock exists but Deadlock Exception was thrown.");
    }
  ...
  @Test
  @Category(StudentTestP3.class)
  public void deadlockTest1() throws InterruptedException {
    final LockManager lockMan = new LockManager();
    AsyncDeadlockTesterThread thread1 = new AsyncDeadlockTesterThread(new Runnable() {
      public void run() {
        lockMan.acquireLock("A", 1, LockManager.LockType.EXCLUSIVE);
    ...
    try {
      ...
      assertTrue("thread2 should wait", thread2.isAlive()); //T2 should not be blocked on B
      assertTrue("thread4 shoudl wait", thread4.isAlive());
      assertFalse("thread5 should not be blocked", thread5.isAlive());
    } catch (DeadlockException d) {
      fail("No deadlock exists but Deadlock Exception was thrown.");
    }
  ...
  @Test
  @Category(StudentTestP3.class)
  public void no_deadlockTest1() throws InterruptedException {
    final LockManager lockMan = new LockManager();
    AsyncDeadlockTesterThread thread1 = new AsyncDeadlockTesterThread(new Runnable() {
      public void run() {
        lockMan.acquireLock("A", 1, LockManager.LockType.EXCLUSIVE);
    ...
    try {
      ...
      thread6.join(100);
      thread6.test();
      assertFalse("thread6 should not be blocked", thread6.isAlive());
    } catch (DeadlockException d) {
      fail("Deadlock Exception not thrown.");
    }
  }
